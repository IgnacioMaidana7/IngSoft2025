...
class TransferenciaListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # Solo admins pueden ver transferencias
        if hasattr(user, 'supermercado'):
            # Es un empleado, no puede ver transferencias
            return Transferencia.objects.none()
        
        # Es un admin, puede ver sus transferencias
...
class TransferenciaDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        # Solo admins pueden acceder a transferencias
        if hasattr(user, 'supermercado'):
            return Transferencia.objects.none()
...
def confirmar_transferencia(request, transferencia_id):
    ...
    try:
        ...
        if hasattr(request.user, 'supermercado'):
            return Response({
                'success': False,
                'error': 'Solo los administradores pueden confirmar transferencias'
